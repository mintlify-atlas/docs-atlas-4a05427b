...
|----------|-----------|-------------|
| `print` | `print(...)` | Prints all arguments concatenated, separated by nothing |
| `type` | `type(value)` | Returns the Hades type name of `value` as a `str` |
| `len` | `len(value)` | Returns the length of a `str` or `list` |
| `pow` | `pow(base, exp)` | Raises `base` to the power of `exp` |
...
<Tip>
  `print` accepts any number of arguments and joins them without a separator. Pass explicit space strings (`' '`) or newline characters (`'\n'`) when you need formatting between values.
</Tip>
