...
Skips the remainder of the current loop body and jumps to the next iteration. In a `for` loop, the update expression (`i++`) still runs. In a `while` loop, execution resumes at the condition check.
...
Exits the innermost enclosing loop immediately, skipping any remaining iterations.
...
Unconditionally transfers execution to a named label defined elsewhere in the same scope. Labels are identifiers followed by a colon (`:`). This is a low-level control mechanism — prefer loops and functions for most use cases.

<Warning>
`_goTo` bypasses the normal structured control flow. Jumping into the middle of a loop or conditional block can cause unpredictable behaviour. Use sparingly.
</Warning>
...
Returns a value from a function. When used after `func … =>` in a signature it specifies the return type; when used as a statement inside a function body it returns the given expression and exits the function.
