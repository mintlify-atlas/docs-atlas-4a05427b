...
5. Everything else (lists, structures, …) → delegates to Python's `bool()`, which means non-empty collections are truthy
...
| Type | Truthy | Falsy |
|------|--------|-------|
| `nothing` | *(never truthy)* | all `nothing` values |
| `bool` | `TRUE` | `FALSE` |
| `int` | any value `≠ 0` | `0` |
| `float` | any value `≠ 0.0` | `0.0` |
| `str` | any non-empty string | `''` (empty string) |
| `record` | any non-empty record | `""` (empty record) |
| `list` | any non-empty list | `[]` (empty list) |
| `structure` | any structure with at least one field set | structure with no values |
