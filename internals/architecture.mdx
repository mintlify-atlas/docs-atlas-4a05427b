---
title: "Hades Interpreter Architecture and Module Layout"
...
<CardGroup cols={2}>
  ...
  <Card title="modules/ast_nodes.py" icon="diagram-project">
    All AST node dataclasses live here — literals, operations, control-flow nodes, `FuncNode`, `ProgramNode`, and the runtime objects `HadesFunction`, `HadesClass`, and `HadesInstance`. Each dataclass carries a reference to the originating `Token` so the interpreter can report accurate source positions.
  </Card>
  ...
  <Card title="modules/scope.py" icon="layer-group">
    The `Scope` class: a `dict`-backed variable environment with `get()`, `set()`, `declare()`, and `has()` methods. `get()` and `set()` both walk the `parent` chain automatically, so inner scopes can read and mutate variables declared in outer scopes.
  </Card>
